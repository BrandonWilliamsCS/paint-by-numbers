export class Color {
    ...
    private static hexStringPattern = /^#?([0-9a-f]{1,2})([0-9a-f]{1,2})([0-9a-f]{1,2})$/i;

    // store a hard-coded sequence to use for "distinct" colors for convenience.
    // prettier-ignore
    ...
    public static fromHexString(hexString: string): Color {
        const result = Color.hexStringPattern.exec(hexString);
        if (!result) {
    ...
    public static getSequenceColor(i: number): Color {
        return Color.sequence[i % Color.sequence.length];
    }
}
